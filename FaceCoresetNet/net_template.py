...
class TransformerDecoderLayerOrig(Module):
    ...
    def forward(self,core_template, norm_encoding_core_template, template_features, norm_encoding_template):
        ...
        #q = self.norm1(q + torch.nn.functional.scaled_dot_product_attention(q, q, q))
        template_with_norm_enc = template_features + norm_encoding_template
        k = self.norm1(template_features + self._sa_block1(template_with_norm_enc, None, None, None))
        v = template_features
        #q = self.norm2(core_template + self._my_mha_block1(q, k, v))

        q = core_template + torch.nn.functional.scaled_dot_product_attention(q, k, v)
...
class TimeDistributed(LightningModule):
    ...
    def forward(self, x):
        ...
        features = self.module.forward(x_reshape)
