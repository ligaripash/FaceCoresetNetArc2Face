...
class TemplateBatch(object):
    ...
    def create_clip(self, anchor_image, clip_size, do_aug_mix=True):
        ...
        for i in range(clip_size):
            ...
            #frame = np.unsqueeze(frame, dim=0)
            clip.append(frame)

        clip = np.concatenate(clip, axis=0)
        return clip
    ...
    def create_fixed_size_template(self, template_images, output_template_size):
        ...
        #template = [item for sublist in fixed_size_template for item in sublist]
        template = np.concatenate(fixed_size_template, axis=0)
    ...
    def __call__(self, batch):
        ...
        if self.mode == 'train':
            for template_images, target in batch:
                ...
                fixed_size_template = fixed_size_template[np.newaxis, :]
        ...
        templates = np.concatenate(templates, axis=0)
    ...
    def __getitem__(self, index):
        ...
        for template_sample in template:
            ...
            if 'WebFace' in self.root or 'webface' in self.root:
                ...
                #Gil - in arcface model from arc2face the images are read as BGR but later swapped back to RGB
                #template_image = Image.fromarray(np.asarray(template_image)[:, :, ::-1])
                template_image = Image.fromarray(np.asarray(template_image))
